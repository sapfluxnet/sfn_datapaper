...
**Figure S2.** Structure of sfn_data objects, which are based on the S4 class. Boxes in the figure represent different slots where data are stored.
in R. Each object is identified by the 'si_code', stored as a slot in the object, 
...
'plant_md' depends on the number of plants in the dataset (and this is depicted by the different length of the box). More information on
...
**Table S2.** Description of all metadata variables in SAPFLUXNET datasets.

**Table S3** Data checks implemented in the first level of data quality control
(QC1).
...
**Table S4.** Datasets in the SAPFLUXNET database identified by numeric code, 
dataset code and site name. Geographic coordinates and elevation are also shown. Negative values shown for Southern and Western Hemispheres.

**Figure S4.** Detailed geographic distribution of SAPFLUXNET datasets. Datasets are labelled by dataset number in Table S4.

**Table S5.** Number of plants per species present in the SAPFLUXNET database.

**Table S6.** Number of plants per genus present in the SAPFLUXNET database.

**Table S7.** Number of species per dataset in the SAPFLUXNET database.

**Figure S5.** Wordcloud of treatments at the stand (a) and at the plant (b) levels.
...
sapfluxnetr:::.metadata_architecture() %>%
  ...
  width(width=c(2.5,2.25,1.2,1.2)) %>%
   align(part='header',align='left') %>% 
    align(part='body',align='left') %>% 
...
readxl::read_xlsx('resources/table_QC2_checks.xlsx',1) %>% 
  ...
  height(height=c(rep(0.5,6),0.7)) %>% 
   set_caption('Table S3. Data checks implemented in the QC2 process')
```
...
sfn_sites_nspecies %>% 
  ...
  height(height=c(rep(0.2,176),rep(0.4,3),rep(0.2,23))) %>% 
   set_caption('Table S4. Datasets in the SAPFLUXNET database identified by 
...
data.frame(sfn_species_ntrees_reduced[1:58,],
  ...
  flextable::set_caption('Table S5. Number of plants per species in the SAPFLUXNET database')
...
data.frame(sfn_genus_ntrees[1:32,],
  ...
  italic(j=c(1,3,5)) %>% 
  height(height=.2) %>% 
 flextable::set_caption('Table S6. Number of plants per genus in the SAPFLUXNET database')
...
data.frame(sfn_nspecies_dataset[1:58,],
  ...
  height(height=c(rep(.2,5),0.4,rep(.2,12),0.4,rep(.2,33),0.4,rep(.2,5))) %>% 
    add_header_lines() %>% 
flextable::set_caption('Table S7. Number of species per dataset in the SAPFLUXNET database')
