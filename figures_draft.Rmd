...
ntrees_genus_top <- ntrees_genus_top_data %>% 
...
data_duration_plot <-
  datasets_duration %>% 
  ggplot(., aes( y=index_days,x=reorder(num_code,n),fill = n_trees_class_f)) +
  geom_tile() +
  coord_flip()+
  ...
  geom_label_repel(data=duration_annotation %>% 
                     filter(rank%in%1:30),
                   aes(x=reorder(num_code,n),y=0.9*n,label=si_code),inherit.aes=FALSE,
                   ylim=c(3800,6000),direction='y',segment.color='darkgrey',force=0.65,
                   size=3.4,label.size=0.03,
                   # WARNING:If size is set to 3, the final plot only shows some, not all, labels ??
                   nudge_x = duration_annotation %>%
                     filter(rank%in%1:30) %>%
                     arrange(desc(n)) %>%
                     pull(num_code)-50)
