...
mean annual temperature and precipitation in a Whittaker diagram showing the classification
of the main terrestrial biomes.

**Figure 3.** Taxonomic distribution of genera and species in SAPFLUXNET, showing 
(a) genera and (b) species with > 50 plants in the database.

**Figure 4.** Distribution of plants in SAPFLUXNET according to taxonomic group, sap flow method 
...
heat dissipatio (HD), heat field deformation (HFD), heat pulse T-max (HPTM), heat ratio (HR), 
stem heat balance (SHB),trunk sector heat balance (TSHB).
...
sap flow data and coloured by the number of plants measured. The 30 longest datasets
...
(c) Venn diagram showing the degree of overlap amongst the 
...
'Temperate rain forest' and 'Tropical rain forest'. Each panel shows the percentage 
of total stand basal area that is covered by sap flow measurements for each species 
in the dataset. Datasets are also coloured by the number of species present. Blue numbers 
...
cowplot::plot_grid(
  ...
  ncol=1, label_size=24, rel_heights=c(0.6,0.4))
...
ntrees_species_top <-
  ...
  geom_text(
    aes(label = n, y = y, x = reorder(genus, n_all)),
     size=6,fontface='bold',colour=viridis::viridis(1),
...
left_join(
 ...
 by='pl_sens_meth') %>% 
  ...
  empty_blanks() %>% 
   set_caption('Table 2. Number of plants using different radial and azimuthal integration approaches, for 
...
read_csv('resources/treatment_table.csv') %>% 
  ...
  align(part='header',align='left') %>% 
   set_caption('Table 3. Number of datasets, plants and species according to treatment at the stand level.')
