...
5000 cm². Panel (b) shows the distribution of SAPFLUXNET stands 
...
```{r sitesmap, echo=FALSE, fig.height=15, fig.width=15, message=FALSE, warning=FALSE, cache=FALSE}
sitesmap_global <- globforest_rec_0_1  +
  geom_point(data=sfn_allsites,aes(x=si_long,y=si_lat),shape=21,color='black',fill='royalblue',alpha=0.5, size = 3)+
...
fig_2 <- cowplot::plot_grid(
  # row 1
  cowplot::plot_grid(sitesmap_global, labels='(a)',label_size=24),
  # row 2
  cowplot::plot_grid(biomes_plot, igbp_plot,labels=c('(b)','(c)'),rel_widths=c(0.6,0.4),label_size=24),
  ncol=1, rel_heights=c(0.5,0.5))

cowplot::save_plot(
  'Fig_2.tiff', fig_2, nrow = 1, base_height = 15, base_width = 15
)

knitr::include_graphics('Fig_2.tiff')
...
ntrees_species_top <-
  ...
  geom_text_repel(
    ...
    # repel options
    nudge_x = 0.5,
    nudge_y = 420 - ntrees_species_top_data_labels %>% filter(genus == 'Quercus') %>% pull(segment_start),
    hjust = 1,
    direction = 'y', min.segment.length = 1000, seed = 25
  ) +
  geom_text_repel(
  ...
  theme(
    ...
    axis.text.x=element_text(size=18,face='italic', angle = 30, vjust = 0.9, hjust = 0.8),
    legend.title = element_blank(),
...
ntrees_genus_top <- 
  ...
  geom_text_repel(
    ...
    size = 4,
    # repel options
    nudge_x = 0.5, nudge_y = c(160, 120, 80),
    hjust = 1,
    direction = 'y', min.segment.length = 1000, seed = 25
  ) +
  geom_text_repel(
...
fig_3 <- cowplot::plot_grid(ntrees_genus_top,ntrees_species_top,
                   label_size= 24,labels=c('a)', 'b)'),ncol=1, nrow=2, align = "v", axis = "b")

cowplot::save_plot(
  'Fig_3.tiff', fig_3, nrow = 1, base_height = 15, base_width = 15
)
...
fig_4 <- sfn_plants_type %>%
  ...
  geom_label_repel(
    stat = "stratum", infer.label = TRUE,size=10,
    direction = 'both',
...
cowplot::save_plot(
  'Fig_4.tiff', fig_4, nrow = 1, base_height = 15, base_width = 15
)
...
plant_violins <-
  ...
  theme(
    ...
    strip.background = element_rect(fill = 'white', colour='darkgray'),
    strip.text = element_text(size = 16, colour = 'black', face = 'bold'),
    legend.position = c(0.85, 0.25), legend.text = element_text(size = 18),
...
sw_inset<- sfn_allplants_long_sw %>% 
  ...
  geom_point(
    data = sfn_allplants_long %>%
        ...
        summarise(median = median(Value, na.rm = TRUE)),
    aes(x = .75, y = median),
    colour = 'white', size = 4, shape = 23, fill = viridis::viridis(1),
    position = position_nudge(x = -0.03)
  )+
  ...
  geom_point(
    data = sfn_allplants_long %>%
        ...
        summarise(median = median(Value, na.rm = TRUE)),
    aes(x = 1, y = median),
    colour = 'black', size = 4, shape = 23, fill = viridis::viridis(1,direction=-1),
    position = position_nudge(x = -0.03)
  )+
...
cowplot::save_plot(
  'Fig_5.tiff', fig_5, nrow = 1, base_height = 15, base_width = 18
)
...
data_duration_plot <-
  ...
  theme(
    ...
    legend.position = c(.2, .15), legend.text = element_text(size = 16),
    legend.title = element_text(size = 18, face = 'bold')
  )
...
fig_6 <- cowplot::plot_grid(
  ...
  label_size=24,hjust=c(-0.6,-0.5),vjust=c(1.1,1.1)
)

cowplot::save_plot(
  'Fig_6.tiff', fig_6, nrow = 1, base_height = 20, base_width = 15
)
...
ncol=1,labels=c('a)','b)','c)'),rel_heights = c(1,1,1),label_size=24)

cowplot::save_plot(
  'Fig_7.tiff', fig_7, nrow = 1, base_height = 20, base_width = 15
)
...
env_freq_plot <-
  ...
  #scale_fill_viridis_d() +
  labs(x = '', y = 'Number of datasets',title='Meteorological variables') +
  theme_bw() +
  theme(
...
soil_pal<- c(viridis::viridis_pal(direction=-1)(5))
...
swc_freq_plot <-
  ...
  labs(x = '', y = 'Number of datasets', title = 'Soil water content\n\n') +
  theme_bw() +
  theme(
     title=element_text(size=18),
    legend.title = element_blank(),
...
upper_grid <- cowplot::plot_grid(
  ...
  label_size = 24, labels=c('a)', 'b)'), rel_widths = c(1, 0.4),vjust=1.2,
...
fig_8 <- cowplot::plot_grid(
  ...
  nrow = 2, label_size = 24, labels = c('', 'c)')
)

cowplot::save_plot(
  'Fig_8.tiff', fig_8, nrow = 1, base_height = 20, base_width = 15
)
...
biomes_bor_and_dry <- cowplot::plot_grid(
  plotlist = biomes_plot_list[1:2], nrow = 1, align = 'h', rel_widths = c(0.3, 1)
)

biomes_grid <- 
  cowplot::plot_grid(
    plotlist = list(biomes_bor_and_dry, biomes_plot_list[[3]], biomes_plot_list[[4]]),
    nrow = 3, align = 'h'
  )
...
cowplot::save_plot(
  'Fig_9.tiff', fig_9, nrow = 1, base_height = 20, base_width = 15
)
